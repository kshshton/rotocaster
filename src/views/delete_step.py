...
class DeleteStep:
    ...
    def __render(self, combobox: CustomComboBox) -> None:
        try:
            ...
            self.__settings.steps_manager.reset_numbers(profile_name=active_profile_name)
            combobox.configure(values=self.__settings.steps_manager.sequence(profile_name=active_profile_name))
            combobox.set(self.__settings.steps_manager.last_step_number(profile_name=active_profile_name))
