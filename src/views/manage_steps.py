...
class ManageSteps:
    ...
    def __update_profile_name(self, entry_box_value: str, combobox: CustomComboBox):
        ...
        if entry_box_value != active_profile and entry_box_value != "":
            ...
            self.__settings.profiles_manager.set_active_profile(
                entry_box_value)
            combobox.configure(
    ...
    def __render(self, master: CTk, combobox: CustomComboBox) -> None:
        try:
            assert self.__settings.profiles_manager.get_active_profile()
            active_profile = self.__settings.profiles_manager.get_active_profile()
            active_profile_steps = self.__settings.profiles_manager.get_active_profile_steps()
            self.__settings.steps_manager.update_steps(
                profile_name=active_profile, steps=active_profile_steps)
            last_step_number = self.__settings.steps_manager.last_step_number(
                profile_name=active_profile)
            self.__settings.steps_manager.set_active_step(last_step_number)
            ...
            steps_combobox = CustomComboBox(
                master=frame,
                state="readonly",
                values=self.__settings.steps_manager.sequence(
                    profile_name=active_profile),
                callback=lambda step: self.__settings.steps_manager.select_step(
                    step),
            ...
            save_button = CustomButton(
                ...
                callback=lambda: (
                    ...
                    self.__settings.save_profile_settings(
                        ProfileStruct(
                            steps=self.__settings.steps_manager.get_steps(
                                profile_name=active_profile),
                        ).to_dict()
                    ),
